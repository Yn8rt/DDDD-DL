...
func init() {
	// 设置内存软限制为2GB，当超过时GC会更积极工作
	// 这不会硬性限制内存，而是提示GC在接近限制时更积极回收
	debug.SetMemoryLimit(2 * 1024 * 1024 * 1024) // 2GB软限制

	// 保持默认GC比率100，避免过度GC影响性能
	// debug.SetGCPercent(100) // 默认值，无需显式设置

	// 后台监控内存，只在内存使用过高时触发主动GC
	go func() {
		for {
...
// monitorAndGC 智能GC：只在内存使用过高时才主动触发
func monitorAndGC() {
	var m runtime.MemStats
	runtime.ReadMemStats(&m)

	// 当堆内存超过1.5GB时，触发主动GC
	if m.HeapAlloc > 1500*1024*1024 {
		runtime.GC()
		debug.FreeOSMemory()
	}
}
...
func InitGlobalBlackFinger() {
	...
	// 尝试从文件加载
	blackFingerFile := "common/config/blackfinger.yaml"
	data, err := os.ReadFile(blackFingerFile)
	if err == nil {
		...
		if err == nil && len(config.Blacklist) > 0 {
			...
			gologger.Info().Msgf("[黑名单] 已从文件加载 %d 个指纹黑名单: %s", len(structs.GlobalBlackFingerMap), blackFingerFile)
			return
		}
	}
	...
	gologger.Info().Msgf("[黑名单] 已加载默认指纹黑名单 (%d 个)", len(structs.GlobalBlackFingerMap))
}
...
func flagAudit() {
	...
	gologger.AuditLogger("MasscanInterface: %v", structs.GlobalConfig.MasscanInterface)
