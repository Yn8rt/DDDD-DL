...
func readConfig(param NucleiParams) {
	...
	// 忽略 nuclei 原始配置的模板目录，以 dddd 项目实际使用的目录为准
	if len(options.Templates) > 0 {
		config.DefaultConfig.SetTemplatesDir(options.Templates[0])
	}
	...
	// 显示正在扫描的统计信息
	options.EnableProgressBar = false // 被动扫描模式禁用，避免 metrics 端点冲突
	// 将统计信息以JSONL格式输出到文件
...
func init() {
	...
	// ShowStackTrace 已在 utils/errors 包中移除
	// if os.Getenv("DEBUG") != "" {
