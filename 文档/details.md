...
./dddd -t '"icp.name="带带弟弟"' -hunter -hmpc 1
```
...
./dddd -t "domain=\"baidu.com\"" -fofa -fmc 10000 (指定最大数量为10000 默认100)
```
...
主动指纹数据库默认路径为`config/dir.yaml`，但您可以使用`-dy`参数指定您喜欢的路径。
